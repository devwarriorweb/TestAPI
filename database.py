...
### --- Método que devuelve un cliente a través de su ID en la Base de Datos  --- ###
async def get_one_customer(id):
	record = await collection.find_one( {"_id": ObjectId(id) } )
	return record

## --- Método que Utiliza collection.find({}) para obtener todos los documentos en la colección.
